...
import { editarProducto } from "../../../Services/ProductoServiceP/ProductoService";
import { BuscarMarcaModal } from "./BuscarMarcaModal";
import { BuscarCategoriaModal } from "./BuscarCategoriaModal";
import Swal from "sweetalert2";

export function EditarProducto({ isOpen, onClose, producto, onSuccess }) {
  ...
  const validarFormulario = () => {
    ...
    if (!categoriaSeleccionada && !formData.categoriaId) {
      nuevosErrores.categoriaId = "La categoría es obligatoria";
    }

    if (!marcaSeleccionada && !formData.marcaId) {
      nuevosErrores.marcaId = "La marca es obligatoria";
    }

    setErrores(nuevosErrores);

    // 🔸 Desplazar hacia arriba si hay errores
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await editarProducto(producto.id, datosEnviar);
      ...
      if (
        ...
        mensajeMostrado = "La categoría seleccionada no existe.";
      } else if (
        mensajeBackend.toLowerCase().includes("marca") &&
        mensajeBackend.toLowerCase().includes("no encontrada")
      ) {
        mensajeMostrado = "La marca seleccionada no existe.";
      } else if (mensajeBackend) {
        mensajeMostrado = mensajeBackend;
      } else {
        mensajeMostrado =
          "Error al actualizar el producto. Por favor, intenta nuevamente.";
      }

      setMensajeError(mensajeMostrado);

      // 🔸 Subir al mensaje de error automáticamente
      containerRef.current?.scrollTo({ top: 0, behavior: "smooth" });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <div 
        ref={containerRef}
        className="fixed right-0 top-0 h-full w-full sm:w-[520px] bg-gray-50 dark:bg-slate-800 shadow-2xl z-50 overflow-y-auto transition-transform"
      >
        <div className="sticky top-0 bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-3">
                <label
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 border rounded-xl transition cursor-pointer ${
                    formData.activo
