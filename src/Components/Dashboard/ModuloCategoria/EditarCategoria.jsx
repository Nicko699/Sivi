...
import { editarCategoria } from "../../../Services/CategoriaServiceP/CategoriaService";
import Swal from "sweetalert2";

export function EditarCategoria({ isOpen, onClose, categoria, onSuccess }) {
  ...
  // Cargar datos de la categoria cuando se abre el modal
  useEffect(() => {
    if (isOpen && categoria) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await editarCategoria(categoria.id, datosEnviar);
      ...
      if (mensajeBackend.toLowerCase().includes("ya existe") || 
        ...
        setMensajeError("Ya existe una Categoria con este nombre. Por favor, elige otro nombre.");
      } else if (mensajeBackend) {
        setMensajeError(mensajeBackend);
      } else {
        setMensajeError("Error al editar la categoria. Por favor, intenta nuevamente.");
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                onChange={handleChange}
                placeholder="Descripción breve de la categoria..."
                maxLength={250}
                rows={5}
